...
By analyzing only dataset **metadata** (avoiding context limits and privacy concerns), the LLMs are tasked with inferring data types, standardizing column names, identifying statistical outliers, and suggesting Tableau/Power BI dashboard layouts. We then pipe the LLM-cleaned data into an **XGBoost Classifier** to see if the LLM's automated cleaning improves downstream Machine Learning performance compared to the raw data.
...
The project is heavily modularized into 4 distinct pipeline stages:
...
4. **`model_evaluator.py`**: Trains an XGBoost classification model to predict target columns. It compares the F1 Score, AUC, and training time of the raw data vs. the LLM-cleaned datasets.
...
- The raw JSON suggestion files from the LLMs (which include the Copilot's BI Dashboard layout suggestions).
